...
class QuickCal_WC_Appointment {
	...
	public static function get( $post_id=null ) {
		if ( !is_integer($post_id) ) {
			$message = sprintf( __('%s integer expected when %s given.', 'booked'), 'QuickCal_WC_Appointment::get($post_id)', gettype($post_id) );
	...
	protected function get_custom_fields_info() {
		...
		$meta_exists = true;
		$separator = '--SEP--';
		$meta_key = '_' . QUICKCAL_WC_PLUGIN_PREFIX . 'cfield_';
	...
	protected function get_calendar() {
		$calendars = wp_get_post_terms($this->post_id, QUICKCAL_WC_TAX_CALENDAR);

		if ( !is_wp_error($calendars) && isset($calendars[0]) ) {
			$calendar_obj = $calendars[0];
			$calendar_page = QuickCal_WC_Functions::get_calendar_page_info($calendar_obj->term_id);
		} else {
			$calendar_obj = false;
			$calendar_page = QuickCal_WC_Functions::get_calendar_page_info();
		}
	...
	protected function get_order() {
		$this->order_id = get_post_meta($this->post_id, '_' . QUICKCAL_WC_PLUGIN_PREFIX . 'appointment_order_id', true);
		return $this;
	}

	protected function get_payment_status() {
		$payment_status = new QuickCal_WC_Appointment_Payment_Status($this->post_id);
