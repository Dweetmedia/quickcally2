...
class QuickCal_WC_Order {
	...
	protected function get_appointments() {
		$this->appointments = get_post_meta($this->order_id, '_' . QUICKCAL_WC_PLUGIN_PREFIX . 'order_appointments', true);
		return $this;
	}
}
...
class QuickCal_WC_Order_Hooks {
	...
	public static function woocommerce_validate_order_items( $order_id ) {
		...
		if ( $cart_appointments['ids'] ) {
			...
			update_post_meta($order_id, '_' . QUICKCAL_WC_PLUGIN_PREFIX . 'order_appointments', $cart_appointments['ids']);
		}
	}
