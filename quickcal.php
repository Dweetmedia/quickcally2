...
define( 'QUICKCAL_VERSION', '1.0.12' );
...
define( 'QUICKCAL_PLUGIN_DIR', untrailingslashit( plugin_dir_path( __FILE__ ) ) );
define( 'QUICKCAL_STYLESHEET_DIR', get_stylesheet_directory() );
define( 'QUICKCAL_PLUGIN_TEMPLATES_DIR', QUICKCAL_PLUGIN_DIR . '/templates/' );
define( 'QUICKCAL_AJAX_INCLUDES_DIR', QUICKCAL_PLUGIN_DIR . '/includes/ajax/' );
define( 'QUICKCAL_BOOKED_PLUGIN', 'booked/booked.php');
define( 'QUICKCAL_LOCO_PLUGIN', 'loco-translate/loco.php');
define( 'QUICKCAL_PLUGIN', 'quickcal/quickcal.php');
define( 'QUICKCAL_NOTICE_DISMISS_PREFIX', 'booked_forever_notice_for_');
...
// Included Add-Ons
require_once QUICKCAL_PLUGIN_DIR . '/includes/add-ons/init.php';

// Booked Updates
// Disabled update check
require_once QUICKCAL_PLUGIN_DIR . '/includes/updates/plugin-update-checker.php';
$booked_update_check = PucFactory::buildUpdateChecker('https://quickcal.co/update/quickcal.json', __FILE__, 'quickcal');

// Booked Mailer Functions
require_once('includes/mailer_functions.php');

if(!class_exists('quickcal_plugin')) {
	class quickcal_plugin {
		/**
		...
		public function __construct() {
			...
			require_once(sprintf("%s/post-types/booked_appointments.php", QUICKCAL_PLUGIN_DIR));
			$booked_appointments_post_type = new booked_appointments_post_type();

			require_once(sprintf("%s/includes/general-functions.php", QUICKCAL_PLUGIN_DIR));
			require_once(sprintf("%s/includes/shortcodes.php", QUICKCAL_PLUGIN_DIR));
			require_once(sprintf("%s/includes/widgets.php", QUICKCAL_PLUGIN_DIR));
			require_once(sprintf("%s/includes/ajax/init.php", QUICKCAL_PLUGIN_DIR));
			require_once(sprintf("%s/includes/ajax/init_admin.php", QUICKCAL_PLUGIN_DIR));
			...
			add_action('parent_file', array(&$this, 'booked_tax_menu_correction'));
                        ...
                        add_action('admin_notices', array(&$this, 'quickcal_loco_notice' ));
                        add_action('wp_ajax_booked_dismiss_notices_forever', array(&$this,'booked_dismiss_notices_forever'));

                        add_action( 'admin_enqueue_scripts', array( &$this, 'booked_dismiss_notice_script' ) );
            
                        add_action('admin_notices', array(&$this, 'quickcal_old_plugin_notice' ));
                        register_activation_hook( WP_PLUGIN_DIR.'/'.QUICKCAL_BOOKED_PLUGIN, array( &$this, 'booked_old_plugin_activate' ));
			...
			if ($admin_email_content && $admin_email_subject):
                ...
                public static function booked_old_plugin_activate(){
                    $styles = 'admin-bar,common';
                    $wp_styles = wp_styles();
                    $wp_styles->concat = $styles;
                    _print_styles();
                    //add other styles here.
                    echo '<style>body{background-color: transparent;}</style>';
                    $message = __('You are using <strong>QuickCal</strong> that replaced the Booked plugin. Deactivate Quickcal if you decide to use the outdated Booked plugin.', 'booked');
                    die($message);
                }
		...
		public static function activate() {
                        //try to disable the booked plugin automatically
                        deactivate_plugins(QUICKCAL_BOOKED_PLUGIN);
                        //if the widget pack is still active, warn the user
                        if (is_plugin_active(QUICKCAL_BOOKED_PLUGIN)) {
                            $message = sprintf(__('The plugin "Booked" is outdated. Disable and remove it to use the QuickCal plugin.', 'booked'), '<strong>', '</strong>');
                            //prevent activation
                            die($message);
                        }
                        else {
                            set_transient( '_quickcal_welcome_screen_activation_redirect', true, 30 );
                        }
                        
			
		}
                public function booked_do_we_show_dismiss_notice_script($key){
                    
                    $cache_key = QUICKCAL_NOTICE_DISMISS_PREFIX. $key;
                    
                    $show = false;
                    
                    if(!get_option($cache_key)){
                        //we show the notice for the first time
                        $show = true;
                    }
                ...
                public function quickcal_old_plugin_notice() {
			$installed = file_exists(WP_PLUGIN_DIR.'/'.QUICKCAL_BOOKED_PLUGIN);
                        if($installed && is_plugin_active(QUICKCAL_PLUGIN) && $this->booked_do_we_show_dismiss_notice_script('quickcal')){
                                $class = 'notice-warning notice is-dismissible booked-dismiss-notice-forever';
                                $message = __('Congratulations on purchasing QuickCal! We noticed that Booked is still installed. Since QuickCal is a replacement for Booked, you can safely deactivate & uninstall the Booked Plugin.', 'booked');
                                
                                printf('<div class="%1$s" data-dismiss-action="booked_dismiss_notices_forever" data-key="quickcal"><p>%2$s</p></div>', esc_attr($class), $message);
                        }
		}
                
                public function booked_dismiss_notice_script() {
                    if($this->booked_do_we_show_dismiss_notice_script('loco') || $this->booked_do_we_show_dismiss_notice_script('quickcal')){
                        wp_enqueue_script( 'jquery' );
                        wp_enqueue_script(
                            'booked-notice',
                            QUICKCAL_PLUGIN_URL . '/assets/js/admin-notice.js',
                            array( 'jquery' ),
                            '1.0'
                        );
                    }
                ...
                public function quickcal_loco_notice() {
                        $installed = file_exists(WP_PLUGIN_DIR.'/'.QUICKCAL_LOCO_PLUGIN);
                        if($installed && $this->booked_do_we_show_dismiss_notice_script('loco')){
                                $class = 'notice-warning notice is-dismissible booked-dismiss-notice-forever';
                                $message = sprintf(__('<p><strong>Important:</strong> You may need to relocate your translations before you uninstall the Booked plugin. Please read our guide %1$shere%2$s.</p>', 'booked'), '<a href="https://link.quickcal.co/migratetranslation" target="_blank">', '</a>');
                                
                                printf('<div class="%1$s" data-dismiss-action="booked_dismiss_notices_forever" data-key="loco"><p>%2$s</p></div>', esc_attr($class), $message);
                        }
		}
                
                public function booked_dismiss_notices_forever() {
                        $key = isset($_POST['key']) ? $_POST['key']: false;
                        if(!$key){
                            return;
                        }
                        add_option(QUICKCAL_NOTICE_DISMISS_PREFIX. $key, true);
		}
		...
		public function admin_init() {
			...
			if ( current_user_can('edit_booked_appointments') && isset($_POST['booked_export_appointments_csv']) ):
			...
			if ($booked_redirect_non_admins):
				if (!current_user_can('edit_booked_appointments') && !defined( 'DOING_AJAX' )){
					...
					$booked_profile_page = quickcal_get_profile_page();
			...
			require_once(sprintf("%s/includes/admin-functions.php", QUICKCAL_PLUGIN_DIR));
			require_once(sprintf("%s/includes/dashboard-widget.php", QUICKCAL_PLUGIN_DIR));
			$this->init_settings();

			// Welcome Screen Redirect
			if ( !get_transient( '_quickcal_welcome_screen_activation_redirect' ) ) {
				return;
  			}

  			delete_transient( '_quickcal_welcome_screen_activation_redirect' );

  			if ( is_network_admin() || isset( $_GET['activate-multi'] ) ) {
  				return;
  			}

  			if (QUICKCAL_WELCOME_SCREEN):
		...
		public function init() {
			...
			require_once(sprintf("%s/includes/functions.php", QUICKCAL_PLUGIN_DIR));

			// Start a session if none is started yet.
			//Added check if headers sent - patch
			if( !headers_sent() && !(session_id() && apply_filters( 'booked_sessions_enabled', true ) ) ){
		        session_start();
		    }

		    // Check to see if the plugin was updated.
			$current_version = get_option('booked_version_check','1.6.20');
			if ( version_compare( $current_version, QUICKCAL_VERSION ) < 0 && !QUICKCAL_DEMO_MODE):
				update_option('booked_version_check',QUICKCAL_VERSION);
				set_transient( '_quickcal_welcome_screen_activation_redirect', true, 60 );
		...
		public function add_to_calendar_check($posts){
			...
			foreach ($posts as $post):
				...
				$profile_shortcode = stripos($post->post_content, '[quickcal-profile');
				$appts_shortcode = stripos($post->post_content, '[quickcal-appointments');
		...
		public function init_settings() {
			...
			foreach($plugin_options as $option_name) {
				register_setting('quickcal_plugin-group', $option_name);
			}
		}
		...
		**********************/
		...
		public function booked_welcome_content(){
			include(sprintf("%s/templates/welcome.php", QUICKCAL_PLUGIN_DIR));
		}
		...
		public function plugin_settings_page() {
			...
			include(sprintf("%s/templates/settings.php", QUICKCAL_PLUGIN_DIR));
		}
		...
		public function admin_pending_list() {
			...
			include(sprintf("%s/templates/pending-list.php", QUICKCAL_PLUGIN_DIR));
		}
		...
		public function admin_calendar() {
			...
			include(sprintf("%s/templates/admin-calendar.php", QUICKCAL_PLUGIN_DIR));
		}
		...
		public function booked_add_pending_appt_bubble() {
			...
			$pending = quickcal_pending_appts_count();
			...
			if (current_user_can('manage_booked_options')):
				...
				if ($booked_booking_type == 'registered' && $booked_redirect_type == 'booked-profile' && !$booked_profile_page && $page != 'booked-welcome'):
					...
					echo '<div class="notice notice-warning" style="line-height:37px; border-left-color:#DB5933;">';
						echo sprintf(esc_html__( 'You need to create a page with the %s shortcode. It is required with your current settings.','booked' ),'<code>[quickcal-profile]</code>').'&nbsp;&nbsp;&nbsp;<a href="'.get_admin_url().'post-new.php?post_type=page">'.esc_html__('Create a Page','booked').'</a>&nbsp;&nbsp;|&nbsp;&nbsp;<a href="'.get_admin_url().'admin.php?page=booked-settings">'.esc_html__('Change Settings','booked').'</a>';
					echo '</div>';
		...
		**********************/
		public function booked_calendars_add_custom_fields($tag) {
			...
			?><div class="form-field">
				...
				?></select>
            <p><?php esc_html_e('This will use your setting from the QuickCal Settings panel by default.','booked'); ?></p>
			</div><?php
		...
		public function booked_calendars_edit_custom_fields($tag) {
			...
			<tr class="form-field">
				...
				<td>
					...
					<span class="description"><?php esc_html_e('This will use your setting from the QuickCal Settings panel by default.'); ?></span>
				</td>
			</tr><?php
		}
		...
		**********************/
		public function booked_save_calendars_custom_fields( $term_id ) {
			if ( isset( $_POST['term_meta'] ) ) {
				$t_id = $term_id;
				$term_meta = get_option( "taxonomy_$t_id", [] );//default array
		...
		**********************/
		...
		public function admin_scripts() {
			...
			if ($current_page == 'booked-settings' || $screen->id == 'dashboard'):
				wp_enqueue_script('booked-serialize', QUICKCAL_PLUGIN_URL . '/assets/js/jquery.serialize.js', array(), QUICKCAL_VERSION);
			endif;
			...
			if (in_array($current_page,$this->booked_screens) || $screen->id == 'dashboard'):
				...
				wp_enqueue_script('spin-js', QUICKCAL_PLUGIN_URL . '/assets/js/spin.min.js', array(), '2.0.1');
				wp_enqueue_script('spin-jquery', QUICKCAL_PLUGIN_URL . '/assets/js/spin.jquery.js', array(), '2.0.1');
				wp_enqueue_script('booked-chosen', QUICKCAL_PLUGIN_URL . '/assets/js/chosen/chosen.jquery.min.js', array(), '1.2.0');
				wp_enqueue_script('booked-fitvids', QUICKCAL_PLUGIN_URL . '/assets/js/fitvids.js', array(), '1.1');
				wp_enqueue_script('booked-tooltipster', QUICKCAL_PLUGIN_URL . '/assets/js/tooltipster/js/jquery.tooltipster.min.js', array(), '3.3.0', true);
				wp_register_script('booked-admin', QUICKCAL_PLUGIN_URL . '/assets/js/admin-functions.js', array(), QUICKCAL_VERSION);
		...
		public function admin_styles() {
			...
			if (in_array($current_page,$this->booked_screens) || $screen->id == 'dashboard'):
				...
				wp_enqueue_style('booked-tooltipster', 	QUICKCAL_PLUGIN_URL . '/assets/js/tooltipster/css/tooltipster.css', array(), '3.3.0');
				wp_enqueue_style('booked-tooltipster-theme', 	QUICKCAL_PLUGIN_URL . '/assets/js/tooltipster/css/themes/tooltipster-light.css', array(), '3.3.0');
				wp_enqueue_style('chosen', QUICKCAL_PLUGIN_URL . '/assets/js/chosen/chosen.min.css', array(), '1.2.0');
				wp_enqueue_style('booked-animations', QUICKCAL_PLUGIN_URL . '/assets/css/animations.css', array(), QUICKCAL_VERSION);
				wp_enqueue_style('booked-admin', QUICKCAL_PLUGIN_URL . '/dist/booked-admin.css', array(), QUICKCAL_VERSION);
			endif;
		...
		public function front_end_scripts() {
			...
			wp_enqueue_script('jquery-ui');
			wp_enqueue_script('jquery-ui-datepicker');
			wp_register_script('booked-atc', QUICKCAL_PLUGIN_URL . '/assets/js/atc.min.js', array(), '1.6.1', true );
			wp_enqueue_script('booked-spin-js', 	QUICKCAL_PLUGIN_URL . '/assets/js/spin.min.js', array(), '2.0.1', true);
			wp_enqueue_script('booked-spin-jquery', QUICKCAL_PLUGIN_URL . '/assets/js/spin.jquery.js', array(), '2.0.1', true);
			wp_enqueue_script('booked-tooltipster', QUICKCAL_PLUGIN_URL . '/assets/js/tooltipster/js/jquery.tooltipster.min.js', array(), '3.3.0', true);
			wp_register_script('booked-functions', QUICKCAL_PLUGIN_URL . '/assets/js/functions.js', array(), QUICKCAL_VERSION, true);

			$booked_redirect_type = get_option('booked_appointment_redirect_type','booked-profile');
			$booked_detect_profile_page = quickcal_get_profile_page();
		...
		public static function front_end_styles() {
			...
			wp_enqueue_style('booked-animations', QUICKCAL_PLUGIN_URL . '/assets/css/animations.css', array(), QUICKCAL_VERSION);
                        ...
                        echo '<style>body{--booked_button_color: '.get_option('booked_button_color','#56C477').';}</style>';

                        wp_enqueue_style('booked-css', QUICKCAL_PLUGIN_URL . '/dist/booked.css', array(), QUICKCAL_VERSION);
                        
                        if(get_option('booked_css_style')==='style_2'){
                                wp_enqueue_style('booked-custom-theme', QUICKCAL_PLUGIN_URL . '/dist/booked-custom-theme.css', array(), QUICKCAL_VERSION);
                        }

			if ( defined('NECTAR_THEME_NAME') && NECTAR_THEME_NAME == 'salient' ):
				wp_enqueue_style('booked-salient-overrides', QUICKCAL_PLUGIN_URL . '/assets/css/theme-specific/salient.css', array(), QUICKCAL_VERSION);
			endif;
			
			if (!isset($_GET['print'])):
				$colors_pattern_file = QUICKCAL_PLUGIN_DIR . '/assets/css/color-theme.php';
				...
				$compressed_booked_color_css = quickcal_compress_css( $booked_color_css );
	...
	} // END class quickcal_plugin
} // END if(!class_exists('quickcal_plugin'))
...
if(class_exists('quickcal_plugin')) {
	...
	// Activation Hook
	register_activation_hook( __FILE__, array('quickcal_plugin', 'activate'));
	...
	$quickcal_plugin = new quickcal_plugin();

	// Add a link to the settings page onto the plugin page
	if(isset($quickcal_plugin)) {

		// Add the settings link to the plugins page
		function quickcal_custom_links($links) {
		...
		add_filter("plugin_action_links_$plugin", 'quickcal_custom_links');

		// Load the Front-End Styles and Color Settings
		add_action('wp_enqueue_scripts', array('quickcal_plugin', 'front_end_styles'));

	}
}

// Localization
function quickcal_local_init(){
	$domain = 'booked';
	$locale = apply_filters('plugin_locale', get_locale(), $domain);
	load_textdomain($domain, WP_LANG_DIR.'/quickcal/'.$domain.'-'.$locale.'.mo');
	load_plugin_textdomain($domain, FALSE, dirname(plugin_basename(__FILE__)).'/languages/');
}
add_action('after_setup_theme', 'quickcal_local_init');
